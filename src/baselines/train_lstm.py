...
def evaluate(model, dl, device):
    model.eval(); ys=[]; yh=[]
    for x,y in dl:
        x=x.squeeze(-1).to(device); ys.append(y.numpy()); yh.append(model(x).cpu().numpy())
...
def main(a):
    ...
    c=torch.load(a.cache,map_location='cpu', weights_only=False); X,y=c['X'],c['y']; tr,va=c['train_idx'],c['val_idx']
    ...
    ck='artifacts/lstm'; os.makedirs(ck,exist_ok=True); last=f'{ck}/last.ckpt'; best=f'{ck}/best.ckpt'
    ...
    with mlflow.start_run(run_name='lstm_baseline'):
        ...
        for ep in range(start, a.epochs):
            tl=train_one_epoch(m,tr_dl,opt,loss_fn,dev); metrics=evaluate(m,va_dl,dev)
...
if __name__=='__main__':
    ...
    p.add_argument('--cache', default='data/cache/fd001_L50_s1_cache.pt'); p.add_argument('--epochs', type=int, default=20)
    p.add_argument('--batch', type=int, default=64); p.add_argument('--hidden', type=int, default=128); p.add_argument('--layers', type=int, default=2)
    p.add_argument('--dropout', type=float, default=0.1); p.add_argument('--lr', type=float, default=1e-3); p.add_argument('--seed', type=int, default=42)
    main(p.parse_args())
